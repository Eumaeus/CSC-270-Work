...
def processForSpelling( c: Corpus ): Corpus = {
...
/* 
		In the file "corpus-tokenize", there is a "tokenizeCorpus" function that
  	splits each passage into word-tokens. This functdion accepts a "function-parameter", the name of a function, which allows for final mapping or filtering. Here, we create "spellCheckCorpus" by sending our Corpus, "corp", to "tokenizeCorpus", asking it to apply the additional function "processForSpelling" before returning the results.
...
/* We don't want to spell-check the same word again and again.
	 ...
	 So we make a sort-of HISTOGRAM, really an INDEX, where we have
	 a list of unique words, to which are attached a Vector of
	 citations.
*/
...
/*  Spell-Check!
		...
		We don't want to lower-case all words, because our dictionary has "Paris" but
		not "paris".			
