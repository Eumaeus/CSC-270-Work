...
def tokenizeCorpus ( c: Corpus, exemplarLabel: String = "wt", corpFunc: Corpus => Corpus = c => c ): Corpus = {
	...
	val tokenizedVector: Vector[CitableNode] = c.nodes.map( n => {
		...
		// 			This uses the 'splitWithSplitter' function in utilities.sc,
		//			which improves on .split by _keeping_ the thing you split on,
		// 			using clever Regex Kung-Fu.
		...
		//			.zipWithIndex creates a Tuple of ( T, Int )
		// 			Here, 'noEmpties' is a Vector[String], so…
		//			… noEmpties.zipWithIndex creates a Vector[ (String, Int) ]
		val tokens: Vector[CitableNode] = noEmpties.zipWithIndex.map( z => {

			// By adding to the citation-component
			//			z._1 would be the token-String, and…
			//			z._2 would the the Int, its index-number
			//			( We do "z._2 + 1" because normal people start counting at 1, not 0)
		...
		tokens
	}).flatten // We don't want a Vector[Vector[CitableNode]], just a Vector[CitableNode]
	...
	//		"corpFunc() is a function passed as a parameter."
	corpFunc(tokCorpus)
...
//		This one takes each token and lower-cases it
...
//		This one takes each token, lower-cases it, and eliminates "stop words", which are boring
